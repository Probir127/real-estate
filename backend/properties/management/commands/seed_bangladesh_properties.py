...
PROPERTIES = [
    ...
    },
    {
        ...
        "price": 150000,  # ৳1.5 Lakh/month rent
    ...
    },
    {
        ...
        "bedrooms": 0,
        "bathrooms": 0,
        "area_sqft": 145200,  # 3.3 acres in sqft
        "garage": 0,
        "year_built": 0,
...
class Command(BaseCommand):
    help = 'Seed database with Bangladesh real estate sample properties'
    ...
    def handle(self, *args, **options):
        ...
        for prop_data in PROPERTIES:
            # Skip if already exists (by title + city)
            if Property.objects.filter(title=prop_data['title'], city=prop_data['city']).exists():
                skipped_count += 1
                continue

            Property.objects.create(agent=agent, **prop_data)
            created_count += 1
            self.stdout.write(f'  [OK] Created: {prop_data["title"][:60]}')

        self.stdout.write(self.style.SUCCESS(
            f'\nDone! Created {created_count} properties, skipped {skipped_count} existing.'
        ))
